...
if !(args ? dlib)
# args: if !(args ? dlib)
then throw "WTF?! ${builtins.toJSON (builtins.attrNames args)}"
else
  (builtins.trace "I GOT CALLED AS A PACKAGE! ${builtins.toJSON (builtins.attrNames args)}" (let
    inherit (args) config dlib lib;

    l = lib // builtins;

    # subsystem = "test";

    # name = l.traceValFn (x: "name: ${x}") (args.name or "<unknown>");
    # subsystem = l.traceValFn (x: "subsystem: ${x}") (args.subsystem or "<unknown>");
