...
in {
  ...
  */
  ...
  # Returns true if given directory contains a file ending with .cabal
    l.any
    (filename: l.hasInfix "control" filename)
  ...
  translateBin = {
    ...
    ''
      ...
      export NAME=$(cat $source/debian/control | head -n 1 | cut -d " " -f2)

      control2json="nix run github:ngi-nix/debnix#control2json"
      map=$(cat ${./debnix.json})
      export deps=$($control2json -- $source/debian/control --map ${./debnix.json})
      echo $deps
