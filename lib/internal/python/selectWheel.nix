# MIT Licensed by @adisbladis
# this is vendored from poetry2nix atm, because it uses pyprojects pep599 instead of poetryLib,
# to get going while testing the design.
# FIXME: just port selectWheel to pyproject.nix
# https://github.com/nix-community/poetry2nix/blame/master/pep425.nix
{
  lib,
  stdenv,
  pep599,
  ...
  findBestMatches = versions: candidates: let
    v = lib.lists.head versions;
    vs = lib.lists.tail versions;
  in
    if (builtins.length versions == 0)
    then []
    else (builtins.filter (file: hasInfix v file) candidates) ++ (findBestMatches vs candidates);
  ...
  isPyVersionCompatible = pyver @ {
    ...
    parseMarker = v: let
      ...
      minor =
        if builtins.stringLength end > 0
        then end
        else "0";
  ...
  selectWheel = files: let
    filesWithoutSources = builtins.filter (file: hasSuffix ".whl" file) files;
    ...
    choose = files: let
      ...
      chooseLinux = x: lib.head (findBestMatches linuxMatches x);
      chooseOSX = x: lib.head (findBestMatches osxMatches x);
    in
      if isLinux
